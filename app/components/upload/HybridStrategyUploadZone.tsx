...
interface HybridStrategyUploadZoneProps {
...
export const HybridStrategyUploadZone: React.FC<HybridStrategyUploadZoneProps> = ({ onUploadSuccess, files, strategyName, displayName }) => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const parsedData = await new Promise<unknown>((resolve, reject) => {
        ...
        worker.postMessage({ type: 'parse_hybrid_strategy', strategyName, arrayBuffer }, [arrayBuffer]);
      });
      ...
      >
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-xs text-brand-text-secondary mt-1 font-sans">Parsing Directional / Non-Directional legs</p>
          </div>
        ) : (
