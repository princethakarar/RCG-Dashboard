...
interface MaxUpsideDownsideUploadZoneProps {
  onUploadSuccess: () => void;
  files: FileDetails[];
}

export const MaxUpsideDownsideUploadZone: React.FC<MaxUpsideDownsideUploadZoneProps> = ({ onUploadSuccess, files }) => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const parsedData = await new Promise<unknown>((resolve, reject) => {
        ...
        worker.postMessage({ type: 'parse_max_upside_downside', arrayBuffer }, [arrayBuffer]);
      });
      ...
      const res = await fetch('/api/upload/max-upside-downside', {
        method: 'POST',
        body: formData,
      });
      ...
      setSuccessMsg(`Successfully uploaded Max Upside / Downside Data`);
      onUploadSuccess();
      router.refresh();
      ...
      >
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-bold text-brand-text-primary">Processing Statistics Data...</p>
            <p className="text-xs text-brand-text-secondary mt-1 font-sans">Parsing Max Upside / Downside</p>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            ...
            <h4 className="text-sm font-bold text-brand-text-primary tracking-tight">
              Drag & Drop Max Upside/Downside Excel File
            </h4>
            <p className="text-xs text-brand-text-secondary mt-1 max-w-xs leading-relaxed">
              Accepts <span className="font-bold">.xlsx</span> files only. Files will be parsed and saved in the statistics database.
            </p>
