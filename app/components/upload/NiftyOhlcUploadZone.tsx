...
interface NiftyOhlcUploadZoneProps {
...
export const NiftyOhlcUploadZone: React.FC<NiftyOhlcUploadZoneProps> = ({ onUploadSuccess, files }) => {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const parsedData = await new Promise<unknown>((resolve, reject) => {
        ...
        worker.postMessage({ type: 'parse_nifty_ohlc', arrayBuffer }, [arrayBuffer]);
      });
      ...
      const res = await fetch('/api/upload/nifty-ohlc', {
        method: 'POST',
        body: formData,
      });
      ...
      >
        ...
        {uploading ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-bold text-brand-text-primary">Processing Nifty OHLC Data...</p>
            <p className="text-xs text-brand-text-secondary mt-1 font-sans">Parsing daily Open / High / Low / Close</p>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            ...
            <h4 className="text-sm font-bold text-brand-text-primary tracking-tight">
              Drag &amp; Drop Nifty OHLC Excel File
            </h4>
            <p className="text-xs text-brand-text-secondary mt-1 max-w-xs leading-relaxed">
              Accepts <span className="font-bold">.xlsx</span> files with columns{' '}
              <span className="font-bold">Date, Open, High, Low, Close</span>. Existing dates are updated in place.
            </p>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              <p className="font-normal">
                ...
                {result.skipped > 0 ? `, ${result.skipped} skipped` : ''} ({formatDateString(result.startDate)} – {formatDateString(result.endDate)}).
              </p>
              {result.skipped > 0 && result.skippedDetails && result.skippedDetails.length > 0 && (
                <ul className="mt-1.5 space-y-0.5 font-normal text-[11px] text-green-700">
